...
def generate_batch_verification_images(output_directory,num_images,length,if_nose,if_line,font_size):
    if not os.path.exists(output_directory):
        os.makedirs(output_directory)

    for  i in tqdm(range(num_images)):
        code=generate_verificaition_code(length=length)
        image=create_verfication_image(code,font_size=random.randint(font_size[0],font_size[1]),if_fill_line=if_line,if_file_nose=if_nose)
        image_path=os.path.join(output_directory,f'{code}.png')
        image.save(image_path)
    print(f'total {num_images} producted in {output_directory}!!! ')
