...
class UserInfo(models.Model):
    ...
    price_policy=models.ForeignKey(verbose_name='价格策略',to='PricePolcy',null=True,blank=True,on_delete=models.CASCADE)

    def __str__(self):
        return self.username

class PricePolcy(models.Model):
    category_choices=(
        (1,'免费版'),
        (2,'收费版'),
        (3,'其他')
    ...
    create_datetime=models.DateTimeField(verbose_name='创键时间',auto_now_add=True)
...
class Transaction(models.Model):
    ...
    user = models.ForeignKey(verbose_name='用户', blank=True,null=True,to='UserInfo',on_delete=models.CASCADE)
    price_policy = models.ForeignKey(verbose_name='价格策略', blank=True,null=True,to='PricePolcy',on_delete=models.CASCADE)
...
class Project(models.Model):
    ...
    creator = models.ForeignKey(verbose_name='创建者', to='UserInfo',blank=True,null=True,on_delete=models.CASCADE)
    create_datetime = models.DateTimeField(verbose_name='创建时间', auto_now_add=True)
...
class ProjectUser(models.Model):
    """ 项目参与者 """
    project = models.ForeignKey(verbose_name='项目', to='Project',blank=True,null=True,on_delete=models.CASCADE)
    user = models.ForeignKey(verbose_name='参与者', to='UserInfo',blank=True,null=True,on_delete=models.CASCADE)
    star = models.BooleanField(verbose_name='星标', default=False)
...
class Wiki(models.Model):
    project=models.ForeignKey(verbose_name='项目',to='Project',on_delete=models.PROTECT)
    title=models.CharField(verbose_name='标题',max_length=32)
...
class FileRepository(models.Model):
    """ 文件库 """
    project = models.ForeignKey(verbose_name='项目', to='Project',null=True,blank=True,on_delete=models.CASCADE)
    file_type_choices = (
        (1, '文件'),
        (2, '文件夹')
    )
...
class IssuesRely(models.Model):

    reply_type_choice=(
        (1,'修改记录'),
        (2,'回复')
    )
    reply_type=models.IntegerField(verbose_name='类型',choices=reply_type_choice)
